...
export default function Carousel({
  ...
  // compute effective width (measured from DOM when available, otherwise baseWidth)
  const effectiveWidth = measuredWidth || baseWidth || 0;
  const itemWidth = effectiveWidth
    ? effectiveWidth - containerPadding * 2
    : 300;
  ...
  // Measure container width to make carousel responsive and fill card width
  useEffect(() => {
              ...
              {item.image ? (
                <div className="carousel-item-media">
                  <img
                    src={item.image}
                    alt={item.title}
                    className="carousel-item-image w-full h-full object-cover"
                  />
                </div>
              ) : (
                <>
